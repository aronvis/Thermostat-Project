...
void lcd_writecommand(unsigned char cmd)
{
    ...
    unsigned char shiftedbits = (cmd << 4);
    lcd_writenibble(shiftedbits);
...
void lcd_writedata(unsigned char dat)
{
    ...
    unsigned char shiftedbits = (dat << 4);
    lcd_writenibble(shiftedbits);
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    unsigned char upperbits = (lcdbits & DATA_BITS);
    PORTD &= ~DATA_BITS;
    PORTD |= upperbits;

    /* Make E signal (PB1) go to 1 and back to 0 */
    unsigned char PB1bits = (1 << PB1);
    PORTB |= PB1bits;
    PORTB |= PB1bits;
    PORTB &= ~PB1bits;
}
